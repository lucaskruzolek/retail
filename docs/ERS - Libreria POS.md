...
* **Presupuesto Comercial:** Propuesta temporal de precios con vigencia estricta de 15 días que **no reserva ni descuenta stock físico** y que vive en una entidad independiente a las ventas.
...
* **Emisión de Presupuestos:** Registro de cotizaciones temporales en la entidad `PRESUPUESTOS` con validez estricta de 15 días y congelamiento del precio unitario pactado (`precio_unitario_pactado`). **No afecta stock ni turno de caja**.
* **Recuperación y Conversión en POS:** Carga del presupuesto por número. Valida que el presupuesto esté vigente y que exista stock físico suficiente. Compara los precios pactados contra los precios de venta actuales de `ARTICULOS`, notificando discrepancias en la UI. Al cobrarse, genera una nueva `VENTA` en el turno activo y marca el presupuesto como `CONVERTIDO`.
...
| **RF-05** | **Vinculación con Catálogos** | Enlace opcional de un artículo propio a un registro de `CATALOGOS_PROVEEDORES` con un *Markup %*. Al actualizarse el costo del distribuidor, recalcula el precio de venta sugerido. |
...
| **RF-07** | **Importación Masiva en Segundo Plano** | Carga de archivos locales Excel (.xlsx) o CSV mapeando columnas de distribuidor (Código, Descripción, Costo) y actualizando precios sin congelar la UI mediante `MiniExcel`. |
...
| **RF-11** | **Emisión de Presupuestos Independientes** | Guardar cotizaciones en la entidad `PRESUPUESTOS` con validez estricta de 15 días, congelando el `precio_unitario_pactado` en `DETALLE_PRESUPUESTOS`. **No descuenta stock ni afecta la caja**. |
| **RF-12** | **Conversión de Presupuesto a Venta y Control de Stock** | Recuperar un presupuesto activo en el POS. Si está vigente, verificar disponibilidad de stock físico ($\text{StockActual} \ge \text{CantidadPresupuestada}$) y comparar precios pactados vs. precios actuales de catálogo. Al cobrarse, registrar una nueva venta en el turno activo y marcar el presupuesto como `CONVERTIDO`. |
